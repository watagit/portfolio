...
- [ ] 1. 移行前の環境確認とベースライン確立
- [x] 1.1 Git作業ディレクトリの状態確認
  - uncommittedな変更がないことを確認（`git status`）
  - 未コミットの変更がある場合は警告を出力
  - _Requirements: 3.3_

- [x] 1.2 移行前のlint/format実行とベースライン記録
...
- [x] 3. biome.json設定ファイルの作成と検証
- [x] 3.1 (P) biome.json基本構造の作成
...
- [x] 3.4 Import順序整理機能の設定
  - `assist.actions.source.organizeImports`を設定
...
- [x] 4. package.jsonスクリプトのBiomeコマンドへの置き換え
- [x] 4.1 (P) lintスクリプトの更新
  - `lint:eslint`スクリプトを`lint:biome`に名称変更
  - コマンドを`eslint .`から`biome check .`に変更
  - `lint`スクリプト（run-s経由）が`lint:biome`, `lint:markup`, `lint:tsc`を順次実行することを確認
  - _Requirements: 4.1, 4.3, 4.4_

- [x] 4.2 (P) formatスクリプトの更新
  - `format`スクリプトのコマンドを`prettier --write .`から`biome format --write .`に変更
  - _Requirements: 4.2_

- [x] 4.3 package.jsonスクリプトの検証
  - `pnpm lint:biome`を実行し、Biomeチェックが正常動作することを確認
  - `pnpm format`を実行し、Biomeフォーマットが正常動作することを確認
  - `pnpm lint`を実行し、Biome/Markuplint/tscが順次実行されることを確認（run-sは順次実行）
  - _Requirements: 4.4, 6.1, 6.2_

### Phase 5: 既存設定ファイルの削除

- [x] 5. 不要設定ファイルの削除
- [x] 5.1 ESLint/Prettier設定ファイルの削除
...
- [x] 6. 全ファイルへのBiomeフォーマット適用と差分レビュー
- [x] 6.1 Biomeフォーマットの全ファイル適用
  - `biome format --write .`を実行
  - 全TypeScript/Astroファイルにフォーマットを適用（34ファイル処理、4ファイル修正）
  - _Requirements: 5.1_

- [x] 6.2 フォーマット差分の確認と検証
  - `git diff`でフォーマット差分を確認
  - ロジック変更がないことを目視確認（スタイルのみの変更）
  - `tsc`で型チェックを実行し、構文エラーがないことを確認（成功）
  - Astroファイルで実験的サポートによるエラーが発生していないかチェック（エラーなし）
  - _Requirements: 5.2, 5.3_

### Phase 7: 開発ワークフローの検証

- [x] 7. 移行後のワークフロー検証とパフォーマンス測定
- [x] 7.1 lintワークフローの検証
  - `pnpm lint`を実行し、エラーなく完了することを確認（成功、6.347秒）
  - Biome/Markuplint/tscが順次実行されることを確認（run-sで順次実行）
  - Biomeがエラーを検出した場合、明確なエラーメッセージが表示されることを確認（確認済み）
  - Astroファイルの誤検知を抑制（noUnusedVariables/noUnusedImports/organizeImports無効化）
  - _Requirements: 6.1, 6.3, 6.4_

- [x] 7.2 formatワークフローの検証
  - `pnpm format`を実行し、全対象ファイルが正しくフォーマットされることを確認（成功、1.410秒）
  - 再度`pnpm format`を実行し、差分が発生しないこと（冪等性）を確認（確認済み）
  - _Requirements: 6.2_

- [x] 7.3 パフォーマンス測定と比較
  - `pnpm lint`の実行時間を計測: 6.347秒（ベースライン9.117秒、**30.4%改善**）
  - `pnpm format`の実行時間を計測: 1.410秒（ベースライン6.227秒、**77.4%改善**）
  - Biome部分のみ: 1.213秒（旧ESLint約4.6秒、**73.6%改善**）
  - Format目標達成、Lint全体は未達だがBiome部分は大幅改善（tsc/markuplintがボトルネック）
  - _Requirements: Non-Functional (Performance)_

- [x] 7.4 CI/CDパイプラインの動作確認
  - `.github/workflows/check.yml`が`pnpm lint`を実行していることを確認（確認済み）
  - package.json更新により自動的にBiomeが呼ばれることを確認
  - _Requirements: 6.1_

### Phase 8: ドキュメント更新

- [x] 8. ドキュメントの更新とBiome公式リンク追加
- [x] 8.1 (P) Steeringドキュメントの更新
  - `.kiro/steering/tech.md`のCode Qualityセクションを更新
  - 「Linter: ESLint」→「Linter/Formatter: Biome」に置き換え
  - 「Formatter: Prettier」の記述を削除
  - Import順序整理機能を`organizeImports`として明記
  - Biome公式ドキュメント（https://biomejs.dev/）へのリンクを追加
  - Common Commandsセクションも更新（biome + markuplint + tsc）
  - _Requirements: 7.1_

- [x] 8.2 (P) READMEの更新（存在する場合）
  - README.mdにlint/format手順の記載を確認
  - ESLint/Prettierの記述なし（Next.jsデフォルトテンプレート）
  - 更新不要
  - _Requirements: 7.2_

- [x] 8.3 ドキュメント更新の検証
  - 更新されたtech.mdをレビュー、記述の正確性を確認（確認済み）
  - Biome公式ドキュメントリンクが正しいことを確認（https://biomejs.dev/）
  - Import順序の記述が一貫していることを確認（サードパーティ → 相対 → エイリアス）
  - _Requirements: 7.3_
